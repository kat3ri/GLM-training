#!/usr/bin/env python3
"""
Main training script for GLM-Image.
Supports both t2i and i2i training modes with reward-based optimization.
"""
import argparse
import yaml
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Train GLM-Image model"
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        "--train_both",
        action="store_true",
        help="Train both AR and DiT components"
    )
...
def main():
    ...
    # Override component if train_both is specified
    if args.train_both:
        config["model"]["component"] = "both"
    ...
    if is_main_process():
        print("=" * 80)
        print("GLM-Image Training")
        print("=" * 80)
        print(f"Mode: {mode}")
        print(f"Component: {config['model']['component']}")
